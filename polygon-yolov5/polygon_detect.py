...
def detect(weights='polygon-yolov5s-ucas.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # print("detection length", len(det))
            print(det.shape)
            if len(det):
...
#            update=False,  # update all models
#            project='runs/detect',  # save results to project/name
#            name='exp_sub',  # save results to project/name
#            exist_ok=False,  # existing project/name ok, do not increment
#            line_thickness=3,  # bounding box thickness (pixels)
#            hide_labels=False,  # hide labels
#            hide_conf=False,  # hide confidences
...
#                         xyxyxyxyn = (torch.tensor(xyxyxyxy).view(1, 8) / gn).view(-1).tolist()  # normalized xyxyxyxy
#                         line = (cls, *xyxyxyxyn, conf) if save_conf else (cls, *xyxyxyxyn)  # label format
#                         with open("/home2/shaon/PolygonObjectDetection/polygon-yolov5/results_coordLabel" + '.txt', 'a') as f:
#                             f.write(('%g ' * len(line)).rstrip() % line + '\n')
